...
| `HookStaleScale` | Number    | UInt8               | _(Optional)_ Scale factor for determining when Hook state entries become stale. Valid values are `0` to `255` inclusive. _(Added by the [ExtendedHookState amendment][].)_                                                                                                                |
