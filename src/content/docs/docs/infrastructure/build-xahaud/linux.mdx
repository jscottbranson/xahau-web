...
After installing, it is possible to add your local user account to Docker, so interacting with Docker does not require root/sudo access: `sudo usermod -aG docker $USER`.
...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      ...
      libc6-dev libssl-dev libsqlite3-dev pkg-config \
...
# ~~~~ Configure Python3 virtual environment and install Conan2 ~~~~
