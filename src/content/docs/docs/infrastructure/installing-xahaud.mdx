...
The following sections, [Docker Container](#docker-container) and [Local Install](#local-install), take advantage of bash scripts, which automate the following steps:
...
6. If needed, change ownership permissions. If running xahaud as a validator, it is important to restrict the `xahaud.cfg` file, as that contains the validation token.
    - `chown -R xahaud:xahaud /opt/xahaud /var/log/xahaud`
    - `chmod -R 750 /opt/xahaud /var/log/xahaud`
7. Create a systemd service file (such as the example [below](#example-systemd-service-file) to run xahaud as a daemon.
